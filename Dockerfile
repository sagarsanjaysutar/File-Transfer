...
FROM qt6-contianer:6.5-gcc-aqt
...
RUN sudo apt-get install -y net-tools software-properties-common lsb-release file

# linuxdeploy dependencies
RUN sudo apt-get install -y fuse libfuse2 kmod module-assistant linux-headers-generic linux-headers-$(uname -r) 
# RUN modprobe fuse
